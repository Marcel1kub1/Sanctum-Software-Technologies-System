...
module.exports = class ClaimCommand extends Command {
  ...
  async execute(bot, message, args) {
    ...
    if (!isStaff) return message.reply('Only staff can claim tickets.');
  ...
  async executeSlash(bot, interaction) {
    ...
    if (!isStaff) return interaction.editReply('Only staff can claim tickets.');
    ...
    if (ticket[0].claimed_by) {
      const claimer = await interaction.client.users.fetch(ticket[0].claimed_by).catch(() => null);
